function Draw (canvas, degree, firstPaint, config = {}) {
  // options
  ...
  if (isMobile) {
    ...
    canvas.addEventListener('mousemove', optimizedMove)(['mouseup', 'mouseleave']).forEach((event) => {
...
Draw.prototype = {
  ...
  getPNGImage (canvas = this.canvas) {
    if (this.clip) {
      canvas = this.getImgRect(canvas)
    }
    if (this.backgroundColor) {
      return this.setBG(canvas, this.backgroundColor)
    }
  ...
  getImgRect (canvas) {
    ...
    let x = 0
    let y = 0
    for (let i = -1, len = imgData.data.length; i < len; i += 4) {
      cur++
      // 获当前像素的X坐标（相对左上角）
      x = cur % w
      y = Math.ceil(cur / w)
    ...
    const nw = maxX - minX
    const nh = maxY - minY
    tmpCanvas.width = nw + (this.padding * 2)
    tmpCanvas.height = nh + (this.padding * 2)
    tmpContext.drawImage(canvas, minX, minY, nw, nh, this.padding, this.padding, nw, nh)
    canvas = tmpCanvas
    return canvas
  },
